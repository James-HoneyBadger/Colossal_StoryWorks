# Adventure Construction Set - Documentation

**Complete documentation for the Adventure Construction Set**

---

## 📖 Quick Navigation

### New to ACS?
👉 Start here: [User Guides → Quick Start](user-guides/QUICKSTART.md)

### Want to play adventures?
👉 Read: [User Guides → User Manual](user-guides/USER_MANUAL.md)

### Want to create adventures?
👉 Read: [User Guides → IDE Guide](user-guides/IDE_GUIDE.md)

### Want to contribute code?
👉 Read: [Developer Guides → Contributing](developer-guides/CONTRIBUTING.md)

### Need API or command reference?
👉 Read: [Reference → Technical Reference](reference/TECHNICAL_REFERENCE.md)

---

## 📂 Documentation Structure

### 📘 User Guides
**For players and adventure creators**

- [**Quick Start Guide**](user-guides/QUICKSTART.md) - Get started in 5 minutes
- [**User Manual**](user-guides/USER_MANUAL.md) - Complete guide for players and creators
- [**IDE Guide**](user-guides/IDE_GUIDE.md) - Visual adventure editor walkthrough
- [**Play in IDE Guide**](user-guides/PLAY_IN_IDE_GUIDE.md) - Testing adventures in the IDE
- [**Example Gameplay**](user-guides/EXAMPLE_GAMEPLAY.md) - Sample gameplay sessions

### 💻 Developer Guides
**For contributors and plugin developers**

- [**Contributing Guide**](developer-guides/CONTRIBUTING.md) - How to contribute to the project
- [**Plugin Development**](developer-guides/PLUGIN_GUIDE.md) - Create custom extensions
- [**DSK Conversion Guide**](developer-guides/DSK_CONVERSION_GUIDE.md) - Convert classic Eamon DSK files
- [**Enhanced Parser Guide**](developer-guides/ENHANCED_PARSER_GUIDE.md) - Natural language parser details
- [**Enhanced Features Guide**](developer-guides/ENHANCED_FEATURES_GUIDE.md) - Advanced game features

### 📚 Reference
**Technical documentation and API reference**

- [**Technical Reference**](reference/TECHNICAL_REFERENCE.md) - Architecture, APIs, internals
- [**Commands Reference**](reference/COMMANDS.md) - All 30+ commands with examples
- [**Architecture**](reference/ARCHITECTURE.md) - System architecture overview
- [**Modular Architecture**](reference/MODULAR_ARCHITECTURE.md) - Module design patterns
- [**Documentation Index**](reference/DOCUMENTATION_INDEX.md) - Complete file index
- [**Documentation Review**](reference/DOCUMENTATION_REVIEW.md) - Documentation verification report
- [**Quick Reference Card**](reference/QUICK_REFERENCE.txt) - Command cheat sheet
- [**DSK Converter Quick Ref**](reference/DSK_CONVERTER_QUICKREF.txt) - DSK conversion commands

### 🗂️ Project Management
**Planning and organization documents**

- [**Project Summary**](project-management/PROJECT_SUMMARY.md) - Overall project overview
- [**Project Organization**](project-management/PROJECT_ORGANIZATION.md) - File organization plan
- [**File Organization**](project-management/FILE_ORGANIZATION.md) - Directory structure
- [**Enhancement Plan**](project-management/ENHANCEMENT_PLAN.md) - Planned improvements
- [**Refactoring Roadmap**](project-management/REFACTORING_ROADMAP.md) - Code refactoring plans

### 🗄️ Legacy
**Historical documents (for reference only)**

- Completed implementation notes
- Old enhancement summaries
- Historical parser test reports
- Archived feature announcements

---

## 🎯 Common Tasks

### I want to...

#### Play an existing adventure
1. Read: [Quick Start](user-guides/QUICKSTART.md)
2. Run: `./scripts/acs-play`

#### Create a new adventure
1. Read: [IDE Guide](user-guides/IDE_GUIDE.md)
2. Run: `./scripts/acs-ide`
3. Follow: [User Manual](user-guides/USER_MANUAL.md)

#### Convert a classic Eamon DSK file
1. Read: [DSK Conversion Guide](developer-guides/DSK_CONVERSION_GUIDE.md)
2. Run: `./scripts/acs-convert <file.dsk>`

#### Develop a plugin
1. Read: [Plugin Guide](developer-guides/PLUGIN_GUIDE.md)
2. Reference: [Technical Reference](reference/TECHNICAL_REFERENCE.md)

#### Contribute code
1. Read: [Contributing Guide](developer-guides/CONTRIBUTING.md)
2. Reference: [Architecture](reference/ARCHITECTURE.md)

#### Look up a command
1. Check: [Commands Reference](reference/COMMANDS.md)
2. Or: [Quick Reference Card](reference/QUICK_REFERENCE.txt)

---

## 📊 Documentation by Role

### Players
- [Quick Start](user-guides/QUICKSTART.md)
- [User Manual](user-guides/USER_MANUAL.md)
- [Example Gameplay](user-guides/EXAMPLE_GAMEPLAY.md)
- [Commands Reference](reference/COMMANDS.md)

### Adventure Creators
- [IDE Guide](user-guides/IDE_GUIDE.md)
- [User Manual](user-guides/USER_MANUAL.md)
- [Enhanced Features](developer-guides/ENHANCED_FEATURES_GUIDE.md)
- [Play in IDE Guide](user-guides/PLAY_IN_IDE_GUIDE.md)

### Plugin Developers
- [Plugin Guide](developer-guides/PLUGIN_GUIDE.md)
- [Technical Reference](reference/TECHNICAL_REFERENCE.md)
- [Architecture](reference/ARCHITECTURE.md)

### Core Contributors
- [Contributing Guide](developer-guides/CONTRIBUTING.md)
- [Technical Reference](reference/TECHNICAL_REFERENCE.md)
- [Architecture](reference/ARCHITECTURE.md)
- [Refactoring Roadmap](project-management/REFACTORING_ROADMAP.md)

---

## 🔗 External Links

- **Main README**: [../README.md](../README.md)
- **Start Here**: [../START_HERE.md](../START_HERE.md)
- **Adventure Library**: [../ADVENTURE_LIBRARY.md](../ADVENTURE_LIBRARY.md)
- **Source Code**: [../src/](../src/)

---

## 📝 Documentation Standards

All documentation follows these principles:
- **Clear hierarchy** - Organized by user role and task
- **Consistent formatting** - Markdown with standard headers
- **Up-to-date** - Matches current codebase (verified Nov 2025)
- **Examples included** - Practical code and command examples
- **Cross-referenced** - Links between related documents

---

**Last Updated**: November 20, 2025  
**Documentation Status**: ✅ Complete and verified

For issues with documentation, see [Contributing Guide](developer-guides/CONTRIBUTING.md).
